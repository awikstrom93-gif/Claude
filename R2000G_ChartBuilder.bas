...
'
' STYLE (as requested):
'   * EXPLICIT palette (fixed RGB), NOT the workbook theme. The workbook is regenerated by
'     Python on every build and always carries the DEFAULT Office theme, so the old
'     ObjectThemeColor approach painted every chart Office blue/orange no matter what palette
'     you set under Page Layout > Colors -- it never picked up the intended scheme. Colours are
'     now set by RGB in SeriesColor() and applied by series POSITION, so the pairing is the same
'     on every chart:  series 1 = R2000G (deep teal, the workbook's identity 1F4E5F),
'     series 2 = S&P 600 Growth (gold), series 3+ = rust / steel / moss / plum. Difference
'     charts (single series) get NEGATIVE bars in red so "R2KG - 600G" reads at a glance.
'     To restyle the whole deck, edit the six colours in SeriesColor() -- nothing else.
'   * Arial throughout.
...
Private Sub ColorSeries(s As Series, idx As Long, typ As String)
    Dim c As Long: c = SeriesColor(idx)
    Select Case typ
        Case "L"
...
'=================================================================== helpers
' THE palette. Fixed RGB (theme-independent) so a Python-regenerated .xlsx always charts in these
' colours. Series are coloured by position: 0 = R2000G, 1 = S&P 600 Growth, 2+ = extra lines.
Private Function SeriesColor(idx As Long) As Long
    Select Case (idx Mod 6)
        Case 0: SeriesColor = RGB(31, 78, 95)      ' 1F4E5F deep teal  -- R2000G (workbook identity)
        Case 1: SeriesColor = RGB(200, 151, 46)    ' C8972E gold       -- S&P 600 Growth
        Case 2: SeriesColor = RGB(122, 59, 46)      ' 7A3B2E rust
        Case 3: SeriesColor = RGB(91, 140, 160)     ' 5B8CA0 steel
        Case 4: SeriesColor = RGB(110, 139, 61)     ' 6E8B3D moss
        Case 5: SeriesColor = RGB(138, 107, 161)    ' 8A6BA1 plum
    End Select
End Function

Private Function NegColor() As Long
    NegColor = RGB(192, 0, 0)                        ' C00000 red -- negative difference bars
End Function
