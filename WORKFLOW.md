...
| Assembly + guard | `r2k_step7_consolidate.py`, **`r2k_report.py`** | step7 assembles the IC workbook (incl. the Data Reliability tab via `r2k_view_reliability`) and injects a self-documenting summary block atop every data tab (title / what / how / method); `r2k_report.py` runs the whole chain + the guard. **The summary is `SUMMARY_ROWS` tall, which shifts each tab's data down — the ChartBuilder macro's `SUMMARY_OFFSET` must match it** so numeric chart header rows still line up (factor tabs self-document and are charted by `@Label`, so they stay unshifted). |
